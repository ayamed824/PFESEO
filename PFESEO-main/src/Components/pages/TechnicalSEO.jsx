...
const TechnicalSEO = () => {
      ...
      if (data) {
        ...
        setTechnicalData(
          transformRawToTechnical(
            ...
            data?.issues || [],
            data?.score_explanations?.technical || ""
          )
        );
      } else {
        setAnalysis(null);
  ...
  // 🔧 Transform raw_data into technical SEO format
  const transformRawToTechnical = (raw, categoryScore, backendIssues = [], explanation = "") => {
    if (!raw) return null;
    
    let imagesArray = [];
    if (Array.isArray(raw.images)) {
      imagesArray = raw.images.filter(i => !i._summary);
    } else if (raw.images?.items) {
      imagesArray = raw.images.items.filter(i => !i._summary);
    }
    ...
    return {
      ...
      performance: {
        loadTime: raw.technical?.load_time_ms 
          ? `${(raw.technical.load_time_ms / 1000).toFixed(2)}s` : 'N/A',
        loadTimeMs: raw.technical?.load_time_ms || 0,
        htmlSize: raw.technical?.html_size_kb 
          ? `${raw.technical.html_size_kb} KB` : 'N/A',
        ...
        title: {
          ...
          recommendation: raw.meta_tags?.title_length < 30 
            ...
            : raw.meta_tags?.title_length > 60 
              ? 'Title too long (max 60 chars)' : null,
        },
        description: {
          ...
          recommendation: raw.meta_tags?.description_length < 120
            ...
            : raw.meta_tags?.description_length > 160
              ? 'Description too long (max 160 chars)' : null,
        },
      ...
      technical: {
        ...
        hasCharset: !!raw.technical?.has_charset || !!raw.technical?.charset,
      ...
      explanation,
      issues: groupTechnicalIssues(backendIssues),
    };
  };

  const groupTechnicalIssues = (backendIssues = []) => {
  ...
  const { categoryScore, performance, metaTags, headings, technical, images, links, issues, explanation } = technicalData;
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
     ...
     {showExportModal && (
  <ExportModal
    section="technical"    // ✅ Backend filtre automatiquement
      ...
      {showProgress && exportTaskId && exportUserId && (
        <div className="fixed top-20 right-6 w-80 bg-white rounded-xl shadow-lg border border-blue-200 p-4 z-40 animate-fade-in">
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-sm font-semibold text-gray-800">📄 Exporting Technical Report...</h4>
            <button 
      ...
      <main className="p-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-3">
              {/* ✅ EXPORT BUTTON - Technical Report Only */}
              <button 
                ...
                title="Export Technical SEO Report"
              >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              {explanation && <p className="text-sm text-gray-700 mt-2 max-w-3xl">{explanation}</p>}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 bg-orange-50 rounded-lg flex items-center justify-center">
                  <i className="fa-solid fa-font text-orange-600"></i>
                </div>
              </div>
              <h3 className="text-gray-600 text-sm mb-1">Charset</h3>
              <p className="text-2xl font-bold text-gray-900">
                {technical?.hasCharset ? 'Present' : 'Missing'}
              </p>
              <p className="text-xs text-gray-500 mt-1">Document encoding signal</p>
            </div>
          </div>
        </section>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { icon: 'fa-font', label: 'Charset Declaration', value: technical.hasCharset },
        ...
        <section className="mb-8">
          ...
          {issues.high.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <div className="w-2 h-6 bg-red-400 rounded mr-3"></div>
                High Priority ({issues.high.length})
              </h3>
              <div className="space-y-4">
                {issues.high.map((issue, idx) => (
                  <div key={issue.id || idx} className="bg-white rounded-xl border-l-4 border-red-400 shadow-sm p-6">
                    <div className="flex items-start space-x-4">
                      <div className="w-10 h-10 bg-red-50 rounded-lg flex items-center justify-center flex-shrink-0">
                        <i className={`fa-solid ${issue.icon || "fa-triangle-exclamation"} text-red-500 text-lg`}></i>
                      </div>
                      <div className="flex-1">
          ...
          {/* No Issues */}
          {issues.critical.length === 0 && issues.high.length === 0 && issues.medium.length === 0 && issues.low.length === 0 && (
