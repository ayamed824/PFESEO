...
const Recommendations = () => {
  ...
  const [analysisUrl, setAnalysisUrl] = useState(""); // 🆕 URL de l'analyse
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      if (latest) {
        // 🆕 Récupérer l'URL et userId pour l'export
        setAnalysisUrl(latest.url || "");
        const userId = latest.user_id || localStorage.getItem("user_id") || "current_user";
      ...
      {showProgress && exportTaskId && exportUserId && (
        <div className="fixed top-20 right-6 w-80 bg-white rounded-xl shadow-lg border border-blue-200 p-4 z-40 animate-fade-in">
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-sm font-semibold text-gray-800">📄 Exporting Recommendations Report...</h4>
            <button 
