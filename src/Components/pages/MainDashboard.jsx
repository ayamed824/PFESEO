...
import ExportModal from "./ExportModal";        // ✅ IMPORT AJOUTÉ
...
const MainDashboard = () => {
  ...
  const [userName, setUserName] = useState("");
   // 🆕 ÉTATS pour l'export
  const [showExportModal, setShowExportModal] = useState(false);     // Afficher/masquer modal
  const [exportTaskId, setExportTaskId] = useState(null);            // ID tâche export
  const [exportUserId, setExportUserId] = useState(null);            // ID utilisateur
  const [showProgress, setShowProgress] = useState(false);           // Afficher progression
...
const fetchUser = async () => {
  ...
  const handleExport = () => {
  console.log("Export clicked!");  // 🆕 AJOUTE console.log bch tchouf
  setShowExportModal(true);
};

  // 🆕 HANDLER : Quand l'export démarre (reçu du modal)
  const handleExportStarted = (taskId) => {
    setExportTaskId(taskId);
    setShowExportModal(false);    // Fermer le modal
    setShowProgress(true);        // Afficher la barre de progression
  };

  // 🆕 HANDLER : Quand l'export est terminé
  const handleExportComplete = () => {
    setShowProgress(false);
    setExportTaskId(null);
    // Optionnel : toast success
  };

  // 🆕 HANDLER : Annuler l'export
...
const score_explanations = latestAnalysis?.score_explanations ?? {};
...
const recommendations = latestAnalysis?.recommendations ?? [];
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* 🆕 DEBUG : Affiche si showExportModal est true */}
    {showExportModal && <p className="text-red-500">MODAL DEVRAIT S'AFFICHER</p>}
      ...
      {showProgress && exportTaskId && exportUserId && (
        <div className="fixed top-20 right-6 w-80 bg-white rounded-xl shadow-lg border border-blue-200 p-4 z-40 animate-fade-in">
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-sm font-semibold text-gray-800">📄 Exporting Report...</h4>
            <button 
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          ...
          <div className="col-span-1 bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            ...
            <p className="text-center text-sm text-gray-600">
              {score_explanations.global || (global_score >= 80 
                ...
                return (
                  <div key={item.key}>
                    ...
                    <p className="mt-2 text-sm text-gray-600 leading-relaxed">
                      {score_explanations[item.key] || `This score reflects the ${item.label.toLowerCase()} checks detected during the crawl.`}
                    </p>
                  </div>
                );
              })}
...
<div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
  ...
  <div className="space-y-3">
    ...
    {/* ❌ BOUTON DOWNLOAD REPORT SUPPRIMÉ */}
  </div>
</div>
        </div>
